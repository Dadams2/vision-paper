...
The empirical results in Section~\ref{sec:evaluation} show that realistic workloads like SIMBA drill-down sessions cannot meaningfully distinguish between recommenders. In this section we provide a conceptual framework that explains this phenomenon, and guides the construction of \emph{diagnostic} benchmarks which do test specific recommender capabilities. The key idea is to view evaluation not only as a property of the model, but as a joint property of the \emph{model} and the \emph{workload}.
