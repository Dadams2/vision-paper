...
Section~\ref{sec:evaluation} showed that realistic workloads such as SIMBA drill-down sessions do not meaningfully distinguish recommenders. Here we provide a conceptual framework for explaining this and for constructing \emph{diagnostic} benchmarks. Evaluation is a joint property of the \emph{model} and the \emph{workload}.
...
Our implicit recommendation task is simple: given the current result $R_q$ and history $\mathcal{H}_q$, rank tuples in $R_q$ by how likely they are to remain relevant in the near future. We operationalise this with a temporal-overlap proxy: a tuple is \emph{relevant} if it reappears in a future result $R_{q+g}$ at gap $g$:
\begingroup\refstepcounter{equation}\label{eq:relevance}\(\text{rel}(t, q, g) = \mathbf{1}[t \in R_{q+g}]\)\,\textup{(\theequation)}\endgroup. This is a conservative proxy for ``the analyst will want to see this again soon'' and supports standard ranking metrics such as $precision$, $recall$, $F1$, and $nDCG@k$.
...
\begin{description}
  \item[Signal fraction $\rho$.]
    The fraction of tuples in a typical result that belong to the target signal. In our association-focused setting, these are the tuples that satisfy embedded association rules. High $\rho$ makes signal easy to hit; low $\rho$ makes it sparse.
  ...
  \item[Noise homogeneity $\sigma^2_{\text{noise}}$.]
    How concentrated the noise is in feature space. Low $\sigma^2_{\text{noise}}$ means noise forms a tight cluster, while signal may be more scattered.
