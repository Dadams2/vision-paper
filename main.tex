...
Figure~\ref{fig:decay_sensitivity} shows the sensitivity to the temporal decay rate $\lambda_r$ for association rules on the adversarial benchmark. Without decay ($\lambda_r=0$), performance degrades as the system over-weights distant patterns. Very high decay ($\lambda_r \geq 0.3$) also hurts performance by discarding useful medium-term patterns.
